...
(() => {
  ...
  function initBrain() {
    ...
    // Brain-like volume: two overlapping ellipsoids ("lobes") with a slight center carve.
    const count = prefersReducedMotion ? 280 : 560;
    const positions = new Float32Array(count * 3);
    const nodes = [];

    const lobeA = { cx: -1.05, cy: 0.05, cz: 0.0, rx: 2.05, ry: 1.55, rz: 1.7 };
    const lobeB = { cx: 1.05, cy: 0.05, cz: 0.0, rx: 2.05, ry: 1.55, rz: 1.7 };

    const insideEllipsoid = (x, y, z, e) => {
    ...
    const bounds = { x: 3.1, y: 2.15, z: 2.35 };
    let i = 0;
    let guard = 0;
    while (i < count && guard < 200000) {
      guard++;
      const x = (Math.random() * 2 - 1) * bounds.x;
      const y = (Math.random() * 2 - 1) * bounds.y;
      const z = (Math.random() * 2 - 1) * bounds.z;

      const inBrain = insideEllipsoid(x, y, z, lobeA) || insideEllipsoid(x, y, z, lobeB);
      if (!inBrain) continue;

      // Carve a subtle center fissure and bottom stem notch for a more brain-like silhouette
      const fissure = Math.exp(-Math.pow(x / 0.42, 2)) * 0.55;
      if (Math.abs(x) < 0.18 && y > -0.2 && z > -1.4) continue;
      if (y < -1.35 && Math.abs(x) < 0.65 && z > -0.4) continue;
      if (y > 1.6 && Math.abs(x) < 0.5 && z < 0.4) continue;
      if (Math.random() < fissure * 0.05) continue;
    ...
    // Connections (lines) with flowing light
    const maxConn = nodes.length * 4;
    const linePos = new Float32Array(maxConn * 2 * 3);
    const lineAlpha = new Float32Array(maxConn * 2);
    const lineGeo = new THREE.BufferGeometry();
    lineGeo.setAttribute("position", new THREE.BufferAttribute(linePos, 3));
    lineGeo.setAttribute("alpha", new THREE.BufferAttribute(lineAlpha, 1));
    ...
    const lineMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        varying float vMix;
        void main(){
          float flow = 0.55 + 0.45*sin(uTime*1.9 + gl_FragCoord.x*0.012 + gl_FragCoord.y*0.008);
          vec3 c = mix(uA, uB, clamp(vMix, 0.0, 1.0));
          gl_FragColor = vec4(c, vAlpha * flow);
        }
      `,
    });
    const lines = new THREE.LineSegments(lineGeo, lineMat);
    ...
    function computeConnections() {
      ...
      for (let a = 0; a < nodes.length; a++) {
        ...
        for (let b = a + 1; b < nodes.length; b++) {
          ...
          linePos[ptr++] = nodes[a].x;
          linePos[ptr++] = nodes[a].y;
          linePos[ptr++] = nodes[a].z;
          linePos[ptr++] = nodes[b].x;
          linePos[ptr++] = nodes[b].y;
          linePos[ptr++] = nodes[b].z;
          lineAlpha[aptr++] = alpha;
          lineAlpha[aptr++] = alpha;
          c++;
          if (c >= maxPerNode) break;
          if (aptr >= lineAlpha.length - 2) break;
        }
        if (aptr >= lineAlpha.length - 2) break;
      }
    ...
    const updateScrollRotation = () => {
      ...
      const delta = (mid - vMid) / Math.max(1, window.innerHeight);
      const clamped = Math.max(-0.7, Math.min(0.7, delta));
      scrollRot.x = clamped * 0.35;
      scrollRot.y = clamped * -0.45;
    };
    ...
    function animate(t) {
      ...
      // scroll + parallax
      const tx = scrollRot.x + (prefersReducedMotion ? 0 : par.y * 0.06);
      const ty = scrollRot.y + (prefersReducedMotion ? 0 : par.x * 0.10);
      group.rotation.x = THREE.MathUtils.lerp(group.rotation.x, tx, 0.06);
      group.rotation.y = THREE.MathUtils.lerp(group.rotation.y, group.rotation.y + ty, 0.05);

      // neural pulse
      ptsMat.size = 0.046 + Math.sin(t / 780) * (prefersReducedMotion ? 0.002 : 0.007);
